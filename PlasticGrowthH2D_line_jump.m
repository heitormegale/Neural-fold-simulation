function [points,cells] = PlasticGrowthH2D_line_jump(K,gamma,g,m,alpha,mu)
    ...
    H=Hamiltonian(ll_a,ll_b,ll_s,[r_a,r_b],kappa,rr,Ncells,k,gamma,lambda,g,A_0,step,0,radius,x_0,x_0,0,0,alpha, mu, ID_initial);
...
%                 dH_a_x(1,n)=dH_a_x(1,n);   
     ...
     % move edges   
        for jj=2:length(r_a)-1
            ...
            r_b(2,(r_b(2,:)<0))=0;
            if 0==abs(x_0_l-radius-r_b(1,jj))
                dH_b_x(jj)=(x_0_l-radius-r_b(1,jj))/dtrr;
                ID_initial(jj)=1;
                 r_b(1,jj)=x_0_l-radius;
            end
            if 0==abs(x_0_r+radius-r_b(1,jj))
                dH_b_x(jj)=(x_0_r+radius-r_b(1,jj))/dtrr;
                ID_initial(jj)=2;
        ...
        H=Hamiltonian(ll_a,ll_b,ll_s,[r_a,r_b],kappa,rr,Ncells,k,gamma,lambda,g,A_0,step,L,radius,x_0_r,x_0_l,delta_L_l,delta_L_r,alpha, mu, ID_initial);
...
function do_it=jump(r_a,r_b,ll_s,ll_b,k,ID_edge,jj,virtual,dtrr,mu,dH_l,L)
grad=dH_l(jj);
interface=-mu*1;
...
energy=-grad^2*dtrr+interface+spring;
if energy<0
do_it=1;
else
do_it=0;  
...
function H=Hamiltonian(ll_a,ll_b,ll_s,r,kappa,rr,N,k,gamma,lambda,g,A_0,step,L,radius,x_0_r,x_0_l,delta_L_l,delta_L_r,alpha, mu, ID_initial)
r_a=r(:,1:length(r)/2);
